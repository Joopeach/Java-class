...
public class FileUtil {
	...
	public static void writefile(String dir, String filename, List<user> userlist) throws Exception {
		...
		if(dirent.exists()) { //해당경로[폴더]가 없으면
		...
		//for문 list활용 : for(임시객체 : 리스트명) : 리스트내 객체수만큼 하나
		for(user temp : userlist) {
			
			//1.회원정보 하나의 문자열에 담기 [ 필드 구분, 회원 구분\n ]
			String outstring = temp.getid()+","+temp.getPassword()+","+temp.getName()+"\n";
			//2.회원 파일쓰기
			byte[] b = outstring.getBytes(); //문자열=>바이트변경
			outputStream.write(b); //파일쓰기
		} //for문 끝
		//4. 스트림 초기화
		outputStream.close(); 
	}
	...
	public static List<user> readReader(Reader reader) throws Exception {
		...
		while( (line = bufferedReader.readLine()) !=null {
									...
									//.read() : 한 문자씩 읽어오기
			// 읽어온 줄 분해
			String[] b = line.split(","); //읽어온 한줄을 , 쉼표 기준으로 분해 => b[0]: 아이디 b[1]:비밀번호 b[2]:이름
